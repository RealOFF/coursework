			...
			(C.version = '2.3.9'),
				 ...
				 */ (C.tokenizer = function (e, t) {
					...
					for (
						...
						var a = s - o;
						if (
							r.charAt(s).match(C.tokenizer.separator) ||
							s == n
						) {
							if (a > 0) {
				...
				(C.Pipeline.load = function (e) {
					...
					return (
						e.forEach(function (e) {
							...
							if (!r)
								throw new Error(
									'Cannot load unregistered function: ' + e,
								);
							t.add(r);
						}),
						t
				...
				(C.Pipeline.prototype.add = function () {
					...
					e.forEach(function (e) {
						C.Pipeline.warnIfFunctionNotRegistered(e),
							this._stack.push(e);
					}, this);
				}),
				...
				(C.Pipeline.prototype.toJSON = function () {
					return this._stack.map(function (e) {
						return (
							C.Pipeline.warnIfFunctionNotRegistered(e), e.label
						);
					});
				}),
				/*!
				...
				(C.Index.prototype.query = function (e) {
					...
					for (a = 0; a < t.clauses.length; a++) {
						var u = t.clauses[a],
							...
							c = C.Set.empty;
						l = u.usePipeline
							? this.pipeline.runString(u.term, {
									fields: u.fields,
							  })
							: [u.term];
						for (var h = 0; h < l.length; h++) {
							...
							for (var m = 0; m < p.length; m++) {
								...
								for (y = 0; y < u.fields.length; y++) {
									...
									if (
										(u.presence ==
											C.Query.presence.REQUIRED &&
											((c = c.union(b)),
											void 0 === s[R] &&
												(s[R] = C.Set.complete)),
										u.presence !=
											C.Query.presence.PROHIBITED)
									) {
										if (
											(n[R].upsert(x, u.boost, function (
												e,
												t,
											) {
												return e + t;
					...
					for (var i = 0; i < n.length; i++) {
						...
						for (var d = 0; d < l.length; d++) {
							...
							null == this.invertedIndex[f][s][r] &&
								(this.invertedIndex[f][s][r] = Object.create(
									null,
								));
							for (
								var m = 0;
						...
						for (var s = 0; s < i.length; s++) {
							...
							for (var u = 0; u < a.length; u++) {
								...
								null == this.metadata[n][o][l]
									? (this.metadata[n][o][l] =
											e.metadata[n][o][l])
									: (this.metadata[n][o][l] = this.metadata[
											n
									  ][o][l].concat(e.metadata[n][o][l]));
							}
						}
					}
				}),
				...
				(C.Query.prototype.clause = function (e) {
					return (
						...
						e.wildcard & C.Query.wildcard.TRAILING &&
							...
							(e.term = e.term + '*'),
						'presence' in e ||
							(e.presence = C.Query.presence.OPTIONAL),
						this.clauses.push(e),
						this
					);
				}),
				...
				(C.QueryLexer.lexTerm = function (e) {
					if (
						(e.width() > 1 &&
							(e.backup(), e.emit(C.QueryLexer.TERM)),
						e.ignore(),
						e.more())
					)
					...
					if (null != t) {
						...
						if (null != r)
							switch (r.type) {
								case C.QueryLexer.TERM:
									return (
										e.nextClause(), C.QueryParser.parseTerm
									);
								case C.QueryLexer.FIELD:
									return (
										e.nextClause(), C.QueryParser.parseField
									);
				...
				return (
					(e.prototype.createComponents = function (e) {
						n.forEach(function (t) {
							e.querySelectorAll(t.selector).forEach(function (
								e,
							) {
		...
		var h = (function (e) {
				...
				return (
					...
					(t.prototype.updateResults = function () {
						if (
							...
							for (
								var t = 0, r = Math.min(10, e.length);
								t < r;
								t++
							) {
					...
					(t.prototype.setLoadingState = function (e) {
						this.loadingState != e &&
							(this.el.classList.remove(
								o[this.loadingState].toLowerCase(),
							),
							(this.loadingState = e),
					...
					(t.prototype.bindEvents = function () {
						...
						this.results.addEventListener('mousedown', function () {
							...
							this.field.addEventListener('keypress', function (
								t,
							) {
			...
			y = (function (e) {
				function t() {
					...
					return (
						...
						(t.height = 0),
						(t.showToolbar = !0),
						(t.toolbar = document.querySelector(
							'.tsd-page-toolbar',
						)),
			...
			v = (function (e) {
				...
				return (
					...
					(t.prototype.onResize = function () {
						for (
							var e, t = 0, r = this.anchors.length;
							t < r;
							t++
						) {
							var n = (e = this.anchors[
								t
					...
					(t.prototype.onScroll = function (e) {
						...
						this.index != i &&
							(this.index > -1 &&
								this.anchors[this.index].link.classList.remove(
									'focus',
								),
							(this.index = i),
							this.index > -1 &&
								this.anchors[this.index].link.classList.add(
									'focus',
								));
					}),
					t
			...
			_ = (function (e) {
				...
				return (
					...
					(t.prototype.onDocumentPointerDown = function (e) {
						if (this.active) {
							if (
								e.target.closest('.col-menu, .tsd-filter-group')
							)
								return;
							this.setActive(!1);
						}
			...
			j = (function (e) {
				...
				return (
					...
					(t.prototype.handleValueChange = function (e, t) {
						...
						var r = this.select.querySelector(
								'li[data-value="' + t + '"]',
							),
